...
export const RecipeForm: React.FunctionComponent<RecipeFormProps> = ({ handleSubmit, fillForm = false, defaultValues }): React.ReactElement => {
    ...
    const onFill = () => {
        formRef.current?.setFieldsValue({
            title: "Apple Pie", servings: 8, readyInMinutes: 130,
        ...
        const sampleDetailedIngredients = ["1/2 cup sugar", "1/2 cup packed brown sugar", "3 tablespoons all-purpose flour", "1 teaspoon ground cinnamon", "1/4 teaspoon ground ginger", "1/4 teaspoon ground nutmeg", "6 to 7 cups thinly sliced peeled tart apples", "1 tablespoon lemon juice", "Pastry for double-crust pie", "1 tablespoon butter"];
        const sampleIngredients = ["sugar", "brown sugar", "flour", "cinnamon sugar", "ginger", "apple"];
        setIngredients([...sampleIngredients])
        setDetailedIngredients([...sampleDetailedIngredients])
    };
    ...
    return (
        <Form labelCol={{ span: 10 }}
            wrapperCol={{ span: 20 }}
            layout="horizontal" ref={formRef} name="control-ref" onFinish={onFinish} validateMessages={validateMessages}>
            ...
            <Form.Item label="Ingredients" name='ingredients' rules={[{ required: true, type: 'array', min: 1, max: 20 }]}>
                <AutoComplete list={ingredients} saveList={(value: string[]): void => setIngredients(value)} form={(value: string[]): void => formRef.current?.setFieldsValue({ ingredients: value })} />
            </Form.Item>
